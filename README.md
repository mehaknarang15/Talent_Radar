...
## 2. Comprehensive Agent Capabilities
...
* **Q1: Fast-Track (Match ≥ 70, Interest ≥ 70):** The ideal hires. High technical alignment combined with high engagement and genuine interest. Move immediately to human interviews.
* **Q2: Needs Nurturing (Match ≥ 70, Interest < 70):** Technically qualified, but demonstrating low engagement, misaligned motivations, or transactional behavior (e.g., Prestige-seekers). Recruiters must actively sell the role or probe for culture fit.
* **Q3: Enthusiastic but Underqualified (Match < 70, Interest ≥ 70):** High energy and authenticity, but missing core technical constraints. Route to junior pipelines or hold for future roles.
* **Q4: Archive (Match < 70, Interest < 70):** Low technical fit and low engagement. Automatically rejected.

## 4. Trade-offs & Architecture
* **Simulated vs. Live Chat:** We simulate the candidate's responses based on their resume context rather than running a live asynchronous loop. **Trade-off:** Sacrifices real-time human spontaneity to allow the system to process and rank hundreds of candidates in seconds.
* **LLM Batching:** Candidates are batched (e.g., 10 per API call) with automated rate-limit handling and exponential backoff. **Trade-off:** Introduces slight execution delays, but ensures strict JSON adherence and prevents context-window degradation.
